...
// ----------------------------------------------------------------------------
// Adapted from llm.c attention implementation
...
{
    // note: re-use inp as a scratch buffer because it is not used in backward
    // inp is (B, T, 3C) QKV
...
int main(int argc, char **argv)
{
    ...
    // run cpu reference
    attention_forward_cpu(out, preatt, att, qkv, B, T, C, NH);
    ...
    for (int j = 0; j < sizeof(block_sizes) / sizeof(int); j++)
    {
        ...
        validate_result(d_preatt, preatt, "preatt", B * NH * T * T);
        validate_result(d_att, att, "att", B * NH * T * T);
    }
