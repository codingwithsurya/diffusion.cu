...
void attention_block_forward(
    ...
    float loss_scale // Add loss scale for FP16
)
{
    // apply groupnorm
    ...
    // apply attention
    attention_forward1(cublas_handle, acts->att_out, acts->qkv2, acts->preatt, acts->att, acts->qkv1, B, H * W, C, C / HS, block_size, loss_scale); // Pass loss scale to attention_forward1
...
)
{
    ...
    attention_backward(cublas_handle, back_acts->buf_B3CHW, back_acts->dqkvr, back_acts->dpreatt, back_acts->datt, back_acts->buf1_BCHW, back_acts->buf2_BCHW, acts->qkv2, acts->att, B, H * W, C, C / HS, loss_scale); // Pass loss scale to attention_backward
    ...
    scale_gradients(grads, num_parameters, loss_scale);
}
...
// Helper function to scale gradients
__global__ void scale_gradients_kernel(float *grads, int n, float scale)
{
...
void scale_gradients(AttentionParams *grads, int n, float scale)
{
    int block_size = 512;
    int num_blocks = ceil_div(n, block_size);
    scale_gradients_kernel<<<num_blocks, block_size>>>(grads->gn_w, n, scale);
}
...
int main(int argc, char **argv)
{
    ...
    int block_size = 512;
    // Set loss scale
    float loss_scale = 128.0;

    printf("Checking forward pass\n");
    attention_block_forward(cublas_handle, B, C, H, W, HS, gn_n_groups, block_size, ¶ms, &acts, loss_scale);
    ...
    attention_block_backward(cublas_handle, B, C, H, W, HS, gn_n_groups, block_size, ¶ms, &acts, &back_acts, &grads, loss_scale);
    ...
    float elapsed_time = benchmark_kernel(
        ...
        cublas_handle, B, C, H, W, HS, gn_n_groups, block_size, ¶ms, &acts, loss_scale);
    ...
    elapsed_time = benchmark_kernel(
        ...
        cublas_handle, B, C, H, W, HS, gn_n_groups, block_size, ¶ms, &acts, &back_acts, &grads, loss_scale);
